...
export function buildPromptSection(cfg: ResolvedRtaConfig): PromptSection {
  const text = [
    ...
    'Skills ' + SKILL_NAMES.join(', ') + ' snapshot the public docs: load one before designing an integration; rta_docs has current page text.',
    'The API key is harness-held as credential ' + cfg.apiKeyEnv + ', injected per call: never ask for it in chat, print it, or put it in client code (no NEXT_PUBLIC_/VITE_ names); it stays server-side in the route adapters. On RTA_KEY_MISSING have the user run /rta setup in the web UI (headless: export ' + cfg.apiKeyEnv + ' before launching dsh).',
    EXAMPLE_AVATAR_ID + ' is a public avatar any key can call.',
    'Credits go per second on air and per generation: rta_avatar_create, rta_loop_set, rta_clips_set and rta_session_mint ask for approval' + (cfg.readOnly ? ' (disabled: readOnly)' : '') + '; release minted sessions via rta_session_release.',
  ].join(' ')
